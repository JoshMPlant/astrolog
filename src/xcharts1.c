/*
** Astrolog (Version 5.42J) File: xcharts1.c
**
...
gi.rAsc = gs.objLeft ? (gs.objLeft < 0 ? 120.0 : -60.0)-gi.rAsc : 0.0;
    ...
    for (i = 0; i <= cObjOpt; i++) if (FProper(i)) /* Figure out where to put planet glyphs. */
            symbol[i] = xplanet[i];
    ...
    if (!FCreateGrid(fFalse))
        return;
    for (i = 0; i <= oNormOpt; i++)if (FNorm(i)&&FProper(i))
            power1[i] = rObjInf[i];
    if (us.nRel <= 0) {
        PlanetPPower();
        for (i = 0; i <= oNormOpt; i++)if (FNorm(i)&&FProper(i))
    ...
    if (TensChart != 0) {
        ...
        for (i = 0; i < 7200; i++) {
            ...
            for (j = 1; j <= cObjOpt; j++) if (FProper(j)) {
                    ...
                    for (k = first; k <= us.nAsp; k++) {
                        ...
                        if (m < GetOrb(1,j,k)){
                            ...
                            valt = (sqrt)(20.0*ppower1[j])*rAspInf[k]*
                                   (1.0 - m/GetOrb(1,j,k)) ;
                            if ((k==aOpp || k==aSqu || k==aSSq || k==aSes || k==aInc ) && (TensChart <= 1))
                                valt = - valt;
        ...
        for (j = cObjOpt; j >= 1; j--)
            for (i = j-1; i >= 0; i--)
                if (grid->n[i][j] && FProper(i) && FProper(j)&& !(FIgnoreS(i)&&FIgnoreS(j))) {
                    k = grid->n[i][j];
                    ip = FNorm(i) ? power1[i] : 2.5;
                    jp = FNorm(j) ? power1[j] : 2.5;
...
void XChartAstroGraph()
{
    ...
    lon = Lon, longm, x, y, z, ad=0.0, oa, am, od, dm, lat;
    ...
    for (i = 1; i <= cObjOpt; i++) if (!ignore[i] || i == oMC) {
            planet1[i] = RFromD(Tropical((i == oMC && !us.fSweHouse) ? is.MC : planet[i]));
    ...
    if (!gs.fAlt) for (i = 1; i <= cObjOpt; i++) if (FProper(i)) {
                xold1 = xold2 = -1000;
...
void XChartGrid()
{
    ...
    for (y = 1, j = oEar-1; y <= gs.nGridCell; y++) {
        do {
            j++;
        } while (!FProper(j) && j <= cObjOpt);
        DrawColor(gi.kiGray);
        DrawDash(0, y*unit, siz, y*unit, !gs.fColor);
        DrawDash(y*unit, 0, y*unit, siz, !gs.fColor);
        if (j <= cObjOpt) for (x = 1, i = oEar-1; x <= gs.nGridCell; x++) {
                do {
                    i++;
                } while (!FProper(i) && i <= cObjOpt);
                if (i <= cObjOpt) {
...
void XChartHorizon()
{
    ...
    lat = RFromD(Lat);
    for (i = 0; i <= cObjOpt; i++) if (!ignore[i] || i == oMC) {
            if (us.fEquator) {
    ...
    for (i = cObjOpt; i >= 0; i--) if (FProper(i))    /* Draw planet's glyph. */
...
void XChartHorizonSky()
{
    ...
    for (i = 0; i <= cObjOpt; i++) if (FProper(i) && i != oMC) {
            lonz[i] = RFromD(Mod(DFromR(lonMC-lonz[i]+rPiHalf)));
            if (!fEquator)
                EquToLocal(&lonz[i], &latz[i], rPiHalf-lat);
            azi[i] = rDegMax-DFromR(lonz[i]);
    ...
    for (i = cObjOpt; i >= 0; i--) if (m[i] >= x1 && FProper(i))  /* Draw glyph. */
            DrawObject(i, m[i], n[i]);
    for (i = cObjOpt; i >= 0; i--) if (x[i] >= y1 && FProper(i)) {
            DrawColor(kObjB[i]);
            DrawSpot(x[i], y[i]);
        }
}
...
void XChartOrbit()
{
    ...
    DrawColor(gi.kiLite);
    DrawEdge(x1, y1, x2, y2);
    for (i = oNormOpt; i >= 0; i--)
        if (FNorm(i)&&FProper(i) && FInRect(m[i], n[i], x1, y1, x2, y2))
            DrawObject(i, m[i], n[i]);
    for (i = oNormOpt; i >= 0; i--)
...
void XChartSector()
{
    ...
    for (i = 1; i <= cSector; i++) {
        ...
        DrawSz(sz, cx+POINT1(unitx, 0.97, PX((real)(i*10+175))),
               cy+POINT1(unity, 0.97, PY((real)(i*10+175)))+
#ifdef KDE
               gi.nScale * 8
#else
               gi.nScaleT*2
    ...
    for (i = 0; i <= cObjOpt; i++)    /* Figure out where to put planet glyphs. */
...
void XChartDispositor()
{
    ...
    cx0 = gs.xWin / 3;
    cy0 = gs.yWin / 2;
    for (i = 1; i <= oNormOpt; i++) if (RulerPlanet(i)) {
    ...
    for (xSub = 0; xSub <= 2; xSub++) {
        ...
        do {
            ...
            for (i = 1; i <= oNormOpt; i++)if (FNorm(i))
                    if (dLev[i] && RulerPlanet(i))
                        cLev[oDis[i]] = fTrue;
            for (i = 1; i <= oNormOpt; i++)if (FNorm(i))    /* A planet isn't a final dispositor */
        ...
        do {
            j = fFalse;
            for (i = 1; i <= oNormOpt; i++) if (FNorm(i)&&RulerPlanet(i)) {
                    if (!dLev[i]) {
                        if (!dLev[oDis[i]])
                            j = fTrue;
...
int mon, X1, Y1, X2, Y2;
{
    ...
    for (x = 0; x <= cWeek; x++) {
        ...
        if (x < cWeek) {
            ...
            DrawSz(sz, x1 + x*xunit + xunit/2, y1 - s*3
#ifdef KDE
                   + FONTY
#endif
                   , dtBottom | dtScale);
