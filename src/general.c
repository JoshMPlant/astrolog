/*
** Astrolog (Version 5.42J) File: general.c
**
...
real d;
{
    return RSgn(d)*(RFloor(RAbs(d))+RFloor(RFract(RAbs(d))*100.0)/60.0+RFract(RAbs(d)*100.0)/36.0); 
}
...
real d;
{
    return RSgn(d)*(RFloor(RAbs(d))+RFloor(RFract(RAbs(d))*60.0)/100.0+0.6*RFract(RAbs(d)*60.0)/100.0); 
}
...
real MinDistance3D(deg1, deg2,lat1,lat2)
real deg1, deg2,lat1,lat2;
{
    real i;
    i = rDegRad*acos(RSinD(lat1)*RSinD(lat2)+RCosD(lat1)*RCosD(lat2)*RCosD(deg2-deg1));
    if (!FBetween(i,0.05,179.95))
        i = rDegRad*sqrt(RCosD(i)*RCosD(i)*RFromD(deg1-deg2)*RFromD(deg1-deg2)+RFromD(lat1-lat2)*RFromD(lat1-lat2));
    return i < rDegHalf ? i : rDegMax - i;
}
...
real MinDifference3D(deg1, deg2,lat1,lat2)
real deg1, deg2;
{
    real i;
    i = rDegRad*acos(RSinD(lat1)*RSinD(lat2)+RCosD(lat1)*RCosD(lat2)*RCosD((deg2-deg1)));
...
int obj1, obj2, asp;
{
    ...
    orb = rAspOrb[asp];
    r = FStar(obj1) ? StarOrb : rObjOrb[obj1];
    orb = Min(orb, r);
    r = FStar(obj2) ? StarOrb : rObjOrb[obj2];
    orb = Min(orb, r);
    if (FNorm(obj1) && FNorm(obj2)) {
        orb += rObjAdd[obj1];
        orb += rObjAdd[obj2];
        if (us.fParallel)
            orb = orb / CoeffPar;
        else if (us.nRel <= rcDual || us.fTransitInf || us.fInfSummary)
            orb = orb /CoeffComp;
    }
    return orb;
}
...
#endif
#ifdef KDE
    if (gi.fiskde && gi.painter)
    {
        KTextLine(FONTX, sz, is.cchCol*gi.w, is.cchRow*FONTY);
        return;
    }
#endif
...
int k;
{
    ...
    if (gi.fiskde)
    {
        if (k < 0)
            k = kLtGray;
        KColor(us.fAnsiColor ? k : (gs.fInverse ? kBlack : kLtGray));
        return;
    }
#endif
...
real deg;
{
    ...
    case 0:
        ...
        deg = Mod(deg + (is.fSeconds ? round/60.0/60.0 : round/60.0));
        sign = (int)deg / 30;
        d = (int)deg - sign*30;
        ...
        if (is.fSeconds) {
            ...
            s = RFract(s)*60.0;
            if (us.fDebug)
                sprintf(&szZod[7], "'%06.03f ", s);
            else sprintf(&szZod[7], "'%02d ", (int)s);
        }
        break;
    ...
    case 1:
        ...
        deg = Mod(deg + (is.fSeconds ? round/4.0/60.0 : round/4.0));
        d = (int)deg / 15;
        ...
        if (is.fSeconds) {
            ...
            s = RFract(s)*60.0;
            if (us.fDebug)
                sprintf(&szZod[7], "'%06.03f ", 60.0*RFract((RFract(deg)*4.0)));
            else sprintf(&szZod[7], ",%02ds", (int)s);
        }
        break;
...
real deg;
{
    ...
    if (!us.fDebug)
           ...
           round=0.0;
    f = deg < 0.0;
    deg = RAbs(deg) + (is.fSeconds ? round/60.0/60.0 : round/60.0);
    d = (int)deg;
    m = (int)(RFract(deg)*60.0);
#ifdef KDE
    ...
    if (is.fSeconds) {
        ...
        s = RFract(s)*60.0;
        if (us.fDebug)
            sprintf(&szAlt[7], "%06.03f\"", s);
       /*     sprintf(&szAlt[7], "%06.03f\"", 60.0*(RFract(60.0*RFract(f ? -deg : deg))));*/
        else sprintf(&szAlt[7], "%02d\"", (int)s);
    }
    return szAlt;
}
...
int mon, day, yea, nFormat;
{
    ...
    if (us.fEuroDate) {
        ...
        case -1:
            ...
            break;
        default:
            sprintf(szDat, "%02d-%02d-%04d", day, mon, yea);
            break;
        }
    } else {
        switch (nFormat) {
...
real tim;
{
    ...
    sec = us.fSeconds ? (int)(100*RFract(rMin) + rRound ) : -1;
    if (sec == 60)
        sec = 0;
...
real lon, lat;
{
    ...
    seci = (int)(60.0 * RFract(rMin) + rRound / 60.0);
    if (seci == 60)
        seci = 0;

    rMin = RFract(RAbs(lat)) * 100.0;
    minj = (int)(rMin + rRound / 600.0);
    secj = (int)(60.0 * RFract(rMin) + rRound / 60.0);
    if (secj == 60)
        secj = 0;

    /*  ch1 = us.fAnsiChar > 1 ? 248 : chDeg1;*/
#ifdef KDE
    ch1 = us.fAnsiChar > 1 ? 176 : chDeg1;
#else
    ch1 = us.fAnsiChar > 1 ? 248 : chDeg1;
#endif
...
char *SzLocationAlt(lon, lat, alt)
real lon, lat;
double alt;
{
    static char szLoc[30];
    int i, j,i2,j2;
    char ch;

    i = (int)(RFract(RAbs(lon))*100.0+(us.fSeconds ? 0 : rRound));
    j = (int)(RFract(RAbs(lat))*100.0+(us.fSeconds ? 0 : rRound));
 /*   i2 = (int)(60.0*(RFract(RAbs(lon))*100.0-(real)i)+rRound);
    j2 = (int)(60.0*(RFract(RAbs(lat))*100.0-(real)j)+rRound);*/
    i2  = (int) (  RFract(100*RAbs(lon))*100.0 + rRound);
    j2  = (int) (  RFract(100*RAbs(lat))*100.0 + rRound);
#ifdef KDE
    ch = us.fAnsiChar > 1 ? 176 : chDeg1;
#else
...
#endif
    if (us.fAnsiChar != 3) {
        sprintf(szLoc, us.fSeconds ? "%3.0f%c%02d'%02d\"%c%3.0f%c%02d'%02d\"%c%5.0f%c%c" : "%3.0f%c%02d%c%c%3.0f%c%02d%c%c%5.0f%c%c",
                RFloor(RAbs(lon)), ch, i, (us.fSeconds ? i2 : chMin), lon < 0.0 ? 'E' : 'W',
                RFloor(RAbs(lat)), ch, j, (us.fSeconds ? j2 : chMin), lat < 0.0 ? 'S' : 'N',
                RAbs(alt) > 9999999999.0 ? 9999 : RAbs(alt)> 9999999 ? RFloor(alt / 1000000 + rRound)
                : RAbs(alt)> 9999 ? RFloor(alt / 1000 + rRound) : RFloor(alt+rRound) ,
                RAbs(alt) > 1999999999999.0 ? 'X' : RAbs(alt) > 9999999999.0 ? 'G'
                : RAbs(alt)> 9999999 ? 'M' : RAbs(alt)> 9999 ? 'k' : 'm',
                RAbs(alt) > 9999 ? 'm' : ' ');
    } else {
        sprintf(szLoc, us.fSeconds ? "%3.0f%c%02d'%02d\"%3.0f%c%02d'%02d\"%c%5.0f%c%c" : "%3.0f%c%02d%c%3.0f%c%02d%c%c%5.0f%c%c",
                RFloor(RAbs(lon)), lon < 0.0 ? 'E' : 'W', i, (us.fSeconds ? i2 : chMin) ,
                RFloor(RAbs(lat)), lat < 0.0 ? 'S' : 'N', j, (us.fSeconds ? j2 : chMin) ,
                /*      RFloor(RAbs(lon)), ch, i, lon < 0.0 ? 'E' : 'W',
                      RFloor(RAbs(lat)), ch, j, lat < 0.0 ? 'S' : 'N',*/ ' ',
                (RAbs(alt) > 9999999999.0) ? 9999 : (RAbs(alt)> 9999999) ? RFloor(alt / 1000000 + rRound)
                : (RAbs(alt)> 9999) ? RFloor(alt / 1000 + rRound) : RFloor(alt+rRound) ,
                RAbs(alt) > 9999999999999.0 ? 'X' : RAbs(alt) > 9999999999.0 ? 'G'
                : (RAbs(alt)> 9999999) ? 'M' : (RAbs(alt)> 9999) ? 'k' : 'm',
                (RAbs(alt) > 9999) ? 'm' : ' ');
    }
    return szLoc;
...
#ifdef DOS
    /* For DOS the array better not cross a segment boundary. */
