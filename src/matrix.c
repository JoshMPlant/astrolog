/*
** Astrolog (Version 5.42J) File: matrix.c
**
...
real MdydszToJulian(mon, day, yea, tim, dst, zon)
int mon, day, yea;
real tim, dst, zon;
{
/*    return (real)MdyToJulian(mon, day, yea) +
           (tim + DegToDec(zon) - DegToDec(dst)) / 24.0;*/
    int fGreg = fTrue;
    real ut;
    ...
    ut= tim + DecToDeg(zon) - DecToDeg(dst) ;
    if (ut > 24) { ut-=24;day--;}   
    if (ut < 0) { ut+=24;day++;}   
    return julday(mon, day, yea, ut, fGreg);
}
...
/*    char sz[cchSzDef];*/
    ...
    AA = Min(AA, 89.5999);        /* Make sure the chart isn't being cast */
    AA = Max(AA, -89.5999);       /* on the precise north or south pole.  */
      oo=DecToDeg(OO);
      aa=DecToDeg(AA);
    ...
    if (fDate) {
        ...
        is.JDr = is.JD + TT/24.0 -rRound;

        if (us.fProgress && us.fDirect && us.fSector){
            arc = ProgToArc(is.JDr,is.JDp);
         if (fSecMotion){
            is.JDr += (fEclipseBackward ? -rSidDay*arc/360.0 : rSidDay*arc/360.0);
            } else {oo=Mod(-rDegHalf+oo+(fEclipseBackward? arc: -arc))-rDegHalf;}
}
        else if (us.fProgress && !us.fSolarArc) {
            /* Determine actual time that a progressed chart is to be cast for. */

            is.JDr += (is.JDp - is.JDr) / us.rProgDay ;
        }
    is.T = (is.JDr-2415020.0)/36525.0;
    }
...
real CuspAscendant()
{
    ...
    Asc = Angle(-RSin(is.RA)*RCos(is.OB)-RTan(RFromD(aa))*RSin(is.OB), RCos(is.RA));
...
bool fNeg;
{
    ...
    for (i = 1; i <= 10; i++) {
        ...
        XS = X*RSin(R1)*RTan(is.OB)*RTan(RFromD(aa) == 0.0 ? 0.0001 : RFromD(aa));
        XS = RAcos(XS);
        if (XS < 0.0)
            XS += rPi;
...
void HouseKoch()
{
    ...
    A1 = RSin(is.RA)*RTan(RFromD(aa))*RTan(is.OB);
    A1 = RAsin(A1);
    for (i = 1; i <= cSign; i++) {
        ...
        X = Angle(RCos(A3)*RCos(is.OB)-KN*RTan(RFromD(aa))*RSin(is.OB), RSin(A3));
...
void HouseCampanus()
{
    ...
    for (i = 1; i <= cSign; i++) {
        ...
        DN = RAtn(RTan(KO)*RCos(RFromD(aa)));
        if (DN < 0.0)
            DN += rPi;
        if (RSin(KO) < 0.0)
            DN += rPi;
        X = Angle(RCos(is.RA+DN)*RCos(is.OB)-RSin(DN)*RTan(RFromD(aa))*RSin(is.OB),
                  RSin(is.RA+DN));
        chouse[i] = Mod(DFromR(X)+is.rSid);
    }
    if (hRevers && PolarMCflip) {
...
void HouseRegiomontanus()
{
    ...
    for (i = 1; i <= cSign; i++) {
        if (hRevers && !PolarMCflip)
            ...
            D = RFromD(60.0+30.0*(real)i);
        X = Angle(RCos(is.RA+D)*RCos(is.OB)-RSin(D)*RTan(RFromD(aa))*RSin(is.OB),
                  RSin(is.RA+D));
        chouse[i] = Mod(DFromR(X)+is.rSid);
    }
    if (hRevers && PolarMCflip) {
...
void HouseTopocentric()
{
    ...
    if (hRevers && !PolarMCflip) {
        ...
        aa = DFromR(P1);
        chouse[3] = CuspTopocentric(30.0) + rPi;
        aa = DFromR(P2);
        chouse[2] = CuspTopocentric(60.0) + rPi;
        aa = DFromR(LT);
        chouse[1] = CuspTopocentric(90.0) + rPi;
        aa = DFromR(P2);
        chouse[6] = CuspTopocentric(120.0);
        aa = DFromR(P1);
        chouse[5] = CuspTopocentric(150.0);
        aa = DFromR(LT);
        for (i = 1; i <= 6; i++) {
        ...
        aa = DFromR(P1);
        chouse[5] = CuspTopocentric(30.0) + rPi;
        aa = DFromR(P2);
        chouse[6] = CuspTopocentric(60.0) + rPi;
        aa = DFromR(LT);
        chouse[1] = CuspTopocentric(90.0);
        aa = DFromR(P2);
        chouse[2] = CuspTopocentric(120.0);
        aa = DFromR(P1);
        chouse[3] = CuspTopocentric(150.0);
        aa = DFromR(LT);
        for (i = 1; i <= 6; i++) {
            chouse[i] = Mod(DFromR(chouse[i])+is.rSid);
            chouse[i+6] = Mod(chouse[i]+rDegHalf);
...
real aber;
{
    ...
    if (us.fPlacalc){
        RecToSph_sp(spacex[ind], spacey[ind],spacez[ind],spacedx[ind],
                    spacedy[ind],spacedz[ind],&planet[ind],&planetalt[ind],&planetdis[ind],
                    &ret[ind],&altret[ind],&disret[ind]);
        planet[ind]*=rDegRad;
        planetalt[ind]*=rDegRad;
    } else {
        RecToPol(spacex[ind], spacey[ind], &ang, &rad);
        planet[ind] = Mod(DFromR(ang) /*+ NU*/ - aber + (us.fPlacalc ? 0 : is.rSid));
        ...
        while (ang < -rDegQuad)
            ang += rDegHalf;
        planetalt[ind] = ang;
        planetdis[ind] = sqrt(spacex[ind]*spacex[ind]+spacey[ind]*spacey[ind]+spacez[ind]*spacez[ind]);
        disret[ind] = sqrt(spacedx[ind]*spacedx[ind]+spacedy[ind]*spacedy[ind]+spacedz[ind]*spacedz[ind]);
    }
}
...
void ComputePlanets()
{
    real helioret[oNormOpt+1], heliox[oNormOpt+1], helioy[oNormOpt+1], helioz[oNormOpt+1];
    real aber = 0.0, AU, E, EA, E1, M, XW, YW, AP, AN, IN, X, Y, G, XS, YS, ZS;
    int ind = oSun, i;
    OE *poe;

    while (ind <= (us.fUranian ? oNormOpt : cPlanet)) {
        if ((ignore[ind] && ind > oSun) || FBetween(ind,uranLo,HypuranLo-1))
            goto LNextPlanet;
...
LNextPlanet:
        ind += (ind == oSun ? 2 : (ind != cPlanet ? 1 : HypuranLo-cPlanet));
    }
    ...
    if (us.objCenter == oSun) {
        if (us.fVelocity)
            for (i = 0; i <= oNormOpt; i++)    /* Use relative velocity */
    ...
    for (i = oEar; i <= (us.fUranian ? oNormOpt : cPlanet);
            i += (i == oSun ? 2 : (i != cPlanet ? 1 : HypuranLo-cPlanet))) {
