/*
** Astrolog (Version 5.42J) File: intrpalt.c
**
...
{
    char sz[cchSzDef], c;
    int i, j, k, l, m, n, jj, kk, SaveStarFlag, DispSS=0, DispSH=0;
    real ip, jp, p;
    ...
    if (InterpretAlt != 4 && InterpretAlt != 6) {
        ...
        for (i = 1; i <= cObjInt; i++) {
            ...
            for (j = 1; j <= cObjInt; j++) {
                ...
                ip = FNorm(i) ? ppower1[i] : 2.5;
                jp = FNorm(j) ? ppower1[j] : 2.5;
                p = (rAspInf[k]*(sqrt)(ip*jp)*
                     (1.0-RAbs((real)(m))/3600.0/GetOrb(i,j,k))*500.0);
    ...
    for (i = starLo; i <= starHi; i++) if (!ignore[oNorm+starname[i-oNorm]]) {
            ...
            k = inhouse[j];
            AnsiColor(kSignA(k));
            sprintf(sz, "  [%2d%s house]", k, szSuffix[k]);
            PrintSz(sz);
            AnsiColor(kDefault);
            ...
            for (jj = 1; jj <= cObjInt; jj++) {
                ...
                sprintf(sz, " - orb: %c%d%c%02d'%02d\"\n",
                        us.fAppSep ? (m < 0 ? 'a' : 's') : (m < 0 ? '-' : '+'),
                                abs(m)/60, chDeg1, (abs(m)%3600)/60, abs(m)%60);
                PrintSz(sz);
            }
...
void TransitInterpret()
{
    ...
    for (i = 1; i <= cObjInt; i++) {
        ...
        for (j = 1; j <= cObjInt; j++) if (!ignore[j]) {
                ...
                sprintf(sz, us.fSeconds ? " - orb: %c%d%c%02d'%02d\"" : " - orb %c%d%c%02d'",
                        us.fAppSep ? (m < 0 ? 'a' : 's') : (m < 0 ? '-' : '+'),
                                abs(m)/3600, chDeg1, (abs(m)%3600)/60, abs(m)%60);
                PrintSz(sz);
                AnsiColor(kDkGreen);
