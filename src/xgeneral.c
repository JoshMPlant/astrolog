/*
** Astrolog (Version 5.42J) File: xgeneral.c
**
...
int obj, x, y;
{
    ...
    if (!gs.fLabel)    /* If we are inhibiting labels, then do nothing. */
        return;
    DrawColor(kObjB[obj]);
    if (obj <= ( us.objNode ? oApo : TerranLo )
#ifdef ASTEROIDS
 || FBetween(obj,oPho,oPho+4)
#else
 || FBetween(obj,CentLo,CentLo+4)
#endif
 ||FCusp(obj) || FBetween(obj,HypuranLo,HypuranHi)) {
#ifdef STROKE
...
int x0, y0;
{
    ...
    while ((chCmd = ChCap(*sz))) {
        ...
        case 'H':
            ...
            deltay = -1;
            break;      /* NorthWest */
            /*    default: deltax= 5; deltay=5; break;  */     /* Shouldn't happen. */
        default:
