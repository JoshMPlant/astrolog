/*
** Astrolog (Version 5.42J) File: astrolog.h
**
...
** Modifications from version 5.41f to 5.42 are by Nicolas Scharnagl.
*/
...
#ifdef X_DISPLAY_MISSING /* from config.h */
#undef MOUSE
#undef X11
#else
#define MOUSE
...
/* else have them but want to compile a commandline version instead.
  You may also try to #define DOS */
...
#define DEFAULT_LONG -8.30595 /* Change these values to the longitude west    */
#define DEFAULT_LAT   47.23035 /* and latitude north of your current location. */
/* Use negative values for east/southern areas. */

#define DEFAULT_ZONE -1.00 /* Change this number to the time zone of your */
/* current location in hours before (west of)  */
/* GMT. Use negative values for eastern zones. */
#define DEFAULT_ALT  410.0 /* Altitude in meters above sealevel */
#define DEFAULT_PRESSURE  1000 /* Atm.Pressure in mbar/hPa */
#define DEFAULT_TEMPERATURE 13.7 /* Temperature in degrees Celsius  */

#ifdef GTOPO               /* This defines which area the file gtopo contains (generated with -ZC
and drawmap program ) */
#define GTOPO_LONFR -10.0  /*Positive = East/North */
#define GTOPO_LONTO 20.0   /* Example is Western Europe */
#define GTOPO_LATFR 30.0
#define GTOPO_LATTO 70.0
#define GTOPO_PREC 120.0  /* Precision in points per degree, 120 for 30arcsecond GTOPO30 */
#endif

#ifdef KDE
#define DEFAULT_TZFILE "/usr/share/zoneinfo/GMT"/* The default zoneinfo file */
#endif
...
#define DEFAULT_SYSTEM 8 /* Normally, Topocentric houses are used (unless the */
/* user specifies otherwise). If you want a       */
/* different default system, change this number   */
/* to a value from 0..9 (values same as in -c).   */

#define DEFAULT_ASPECTS 5 /* Default number of aspects to use in charts,may be overriden in astrolog.dat  */

#define DIVISIONS 48 /* Greater numbers means more accuracy but slower  */
/* calculation, of exact aspect and transit times. */
#ifdef ASTEROIDS
#define DEFAULT_INFOFILE "aastrolog.dat"
#define PLACALC /* asteroids needs placalc */
#else
...
#define WHEELCOLS 17   /* Affects width of each house in wheel display.    */
#ifdef ASTEROIDS
#define WHEELROWS (50+us.nExtraAst/6)   /* Max no. of objects that can be in a wheel house. */
#define MAXINDAY (4800+10*us.nAsp*us.nExtraAst/(10*(1+2*(int)us.nTreshold)))   /* Max number of aspects or transits displayable.   */
#else
#define WHEELROWS 40   /* Max no. of objects that can be in a wheel house. */
#define MAXINDAY 3500   /* Max number of aspects or transits displayable.   */
#endif
...
#define MAXCROSS 1750   /* Max number of latitude crossings displayable.    */
#define CREDITWIDTH 74 /* Number of text columns in the -Hc credit screen. */
#define MAXSWITCHES 128 /* Max number of switch parameters per input line.  */
...
#ifdef KDE
#define chDeg0 (char)(us.fAnsiChar ? 176 : ' ')
#define chDeg1 (char)(us.fAnsiChar ? 176 : ':')
#else
...
#ifdef KDE
#ifndef GRAPH
"If 'KDE' is defined 'GRAPH' must be too"
#endif
#endif
#ifdef MSG
...
#endif
#ifndef KDE
"If 'WIN' is defined 'KDE' must not be as well"
#endif
#ifdef MSG
...
#endif
#ifdef KDE
"If 'MSG' is defined 'KDE' must not be as well"
#endif
#ifdef WIN
...
#ifdef ASTEROIDS
#define ASTEROIDSNR 1
#else
#define ASTEROIDSNR 0
#endif
...
#define szAddressCoren \
  "scharn@bluewin.ch - http://www.multimania.de/masegand"
#ifdef KDE
#define ZoneInfoPath "/usr/share/zoneinfo/"
#endif
...
#define cSign      12
#define MaxNumberedAsteroid 260000
#define MaxNumberedComet 500
#define MaxComet 21
#ifdef ASTEROIDS
#define cObj       4307
#define cObjInt    RiyaHi+33
#else
#define cObj 124        /* 227-103 */
#define cObjInt    cObj
#endif
...
#ifdef ASTEROIDS
#define cStar   1079
#define cStarSmall   79
#define cHard   (AtenHi-CentLo+8)
#else
#define cHard   (AtenHi-CentLo+7)
#define cStar   79
#define cStarSmall   79
#endif
#define cRiya cRiyaT+cRiyaC+cRiyaA+cRiyaX
#define cRiyaT 33
#define cRiyaC 58
#define cRiyaA 27
#define cRiyaX 10
...
#define uranLo  33
#define uranHi  (cObj)
#define TerranLo  oFor
#define TerranHi  oCoasc
#define LunarLo  oNod
#define LunarHi  oApo
#define HypuranLo  (AtenHi+1)
#define HypuranHi  (AtenHi+9)
#ifdef ASTEROIDS
#define CentHi  (uranLo+16)
#define CentLo  (uranLo+4)
#define TrneHi  (uranLo+44)
#define TrneLo  (uranLo+17)
#define AgenHi  (TrneHi+23)
#define AgenLo  (TrneHi+1)
#define MuseHi  (AgenHi+9)
#define MuseLo  (AgenHi+1)
#define ApolHi  (MuseHi+16)
#define ApolLo  (MuseHi+1)
#define AmorHi  (ApolHi+7)
#define AmorLo  (ApolHi+1)
#define AtenHi  (AmorHi+7)
#define AtenLo  (AmorHi+1)
#else
#define AtenHi  (uranLo+3)
#define CentLo  (uranLo+4)
#endif
#define AsteHi  cObj
#define FastLo  oVtx
#define FastHi  TerranHi
#ifdef ASTEROIDS
#define oNormOpt oNorm
#define cObjOpt cObj
#define objMaxOpt (cObj+1)
#define oNorm   cObj
#define RiyaHi  (starHi+cRiya)
#define RiyaLo  (starHi+1)
#define AsteLo  (RiyaHi+1*ASTEROIDSNR)
#else
#define oNorm   HypuranHi
#define oNormOpt oNorm
#define cObjOpt cObj
#define objMaxOpt (starHi+1)
#define AsteLo cObj
#endif
#define starLo  (HypuranHi+1)
#define starHi  (HypuranHi+cStarSmall)
...
#ifdef ASTEROIDS
#define oPho CentLo
#define oCha (CentLo+1)
#define oHyl (CentLo+2)
#define oNes (CentLo+3)
#define oAsb (CentLo+4)
#define oSed (TrneLo+24)
#define oGeo (ApolLo+1)
#define oApollo (ApolLo+3)
#define oMidas (ApolLo+8)
#define oPan (ApolLo+14)
#define oDio (AmorLo+6)
#define oHat (AtenLo+2)
#define oKhu (AtenLo+3)
#endif
...
#define FONTX 6
#define FONTY 11
#define KASTPATH "/kastrolog/"
#ifdef KDE1
#define KASTDATADIR "/.kde/share/apps/kastrolog"
#else
#define KASTDATADIR "/astrolog"
#endif
#define GSTEST(nb, var) r = atof(argv[nb]); if (!FBetween(r, -5.0, 5.0)) \
 { char sz[cchSzDef]; sprintf(sz, \
 transl("Value %f passed to switch %cYm out of range.\n"), r, chSwitch); \
 PrintError(sz); return tcError; } gs.var = (int)(r / 2.54 * 72.0)
#else
#define transl(c) c
#endif
...
#define FItem(obj)    FBetween(obj, 0, cObjOpt)
#define FNorm(obj)    (FItem(obj) && !FStar(obj))
#define FCusp(obj)    FBetween(obj, cuspLo, cuspHi)
#define FAngle(obj)   (FCusp(obj) && (obj)%3 == 0)
#define FMinor(obj)   (FCusp(obj) && (obj)%3 != 0)
#define FStar(obj)    FBetween(obj, starLo, ((!us.fRiyalAst&&us.nStar) ? starLo+cStar-1 : starHi ))
#define FObject(obj)  ((obj) <= LunarHi || (obj) > TerranHi)
#define FThing(obj)   ((obj) <= (us.objNode ? oEP : cThing) || (obj) > (us.objNode ? cuspHi : TerranHi))
#define FNode(obj)   (us.objNode && ((obj) == oEP || (obj) == oVtx || FBetween(obj,TerranLo,TerranHi)))
#define FAst(obj)   (us.objAst && (obj) == oVtx)
...
#define ChDst(dst)    (deltaT==0.0 ? 'E' : (dst == 0.0 ? 'S' : (dst == 1.0 ? 'D' : 'A')))
...
#define chObj5(A) szObjName[A][0], szObjName[A][1], szObjName[A][2], szObjName[A][3],szObjName[A][4]
...
#define FIgnoreS(i) (ignoreS[i] || (i) == us.objCenter)

/*#define FIgnore(i) (ignore[i])
#define FIgnore2(i) (ignore2[i])
#define FIgnore3(i) (ignore3[i])
#define FIgnoreS(i) (ignoreS[i])*/
#define fSouthNode (fAlsoSouthNode || !us.fPlacalc || us.objCenter != oEar)
...
#define SetCIA(ci, M, D, Y, T, S, Z, O, A,N) \
  ci.mon = M; ci.day = D; ci.yea = Y; \
  ci.tim = T; ci.dst = S; ci.zon = Z; ci.lon = O; ci.lat = A; ci.alt = N;
...
#define AllocateNear(p, cb) (p) = malloc(cb)
#define AllocateFar(p, cb)  (p) = malloc(cb)
#define AllocateHuge(p, cb) (p) = malloc(cb)
#define DeallocateNear(p) free(p)
#define DeallocateFar(p)  free(p)
#define DeallocateHuge(p) free(p)
/*#define DeallocateNear(p) free(p)
...
/* The const necessary redefinition */
#ifdef KDE
#undef CONST
#define CONST const
#endif
...
typedef struct _GridInfo { byte  n[objMax][objMax]; int v[objMax][objMax]; } GridInfo;

/*
typedef struct _GridInfoOpt {
  byte  n[objMaxOpt][objMaxOpt];
  short v[objMaxOpt][objMaxOpt];
} GridInfoOpt;
*/
...
typedef struct _UserSettings {
    ...
    _bool fContinued;     /* -Tc */
    _bool fInfSummary;     /* -Ts */
    _int  fDebug;     /* -NB */
#ifdef PLACALC
    _bool fNodeDscPeri;     /* -Nd */
    _bool fNodeApPeri;     /* -Na */
    _bool fNodeMean;     /* -N0 */
    _bool fNodeFocal;     /* -Nf */
    _bool fNodeBary;     /* -Nb */
#endif
    ...
    _bool fEuroDate;    /* -Yd */
    _bool fEuroTime;    /* -Yt */
    _bool fTruePos;     /* -YT Are we going to use true positions  */
    _bool fTopocentric;     /* -YTT Are we going to use topocentric positions  */
    _bool fNoAberration;     /* -YTA Are we going to use Aberration  */
    _bool fNoGravityDeflection;     /* -YTG Are we going to use gravitational Deflection  */
    _bool fBarycentric;     /* -YTB Are we going return barycentric Sun/Earth */
    ...
    _bool fSidEclDate;     /* -st Are we going to use Ecliptic of fixed date */
    _bool fSidSolSysPlane;     /* -ss Are we going to project on the solar system plane */
    _bool fGalPlane;     /* -sg Are we going to project on the galactic plane */
    int fSiderealMode;     /* -sm Which sidereal definition to use */
    int ftransIter;     /* -Ys transitverlauf  */
    int ftransStep;     /* -Ys transitverlauf schritte  */
    _bool fSmartCusp;   /* -YC */
    _bool fClip80;      /* -Y8 */
    _bool fWriteOld;    /* -Yo */
    ...
    int   nDivision;    /* -d */
    int   nScreenWidth; /* -I */
    real  nStationOrb;       /* -YOs Limit for Station */
    real  nEpsilon;       /* Obliquity Ecliptic  */
    real  nSortDial;       /* -Nd  */
    real  nDegFrom;       /* -Nr <x><> */
    real  nDegTo;       /* -Nr  <><x>*/
    real  nDegOrb;       /* -NR  <><x>*/
    int  nDegObj;       /* -Nr  <x><>*/
    int  nLocationCount;       /* -zL0  <><x>*/
    int  nEclipseCount;       /* -zL0  <><x>*/
    real  nTreshold;       /* -NT  x*/
    real  nTargetDistance;       /* -Nt  x*/
    real  nSidEclDate;       /* -st  */
    real  dstDef;       /* -z0 */
    real  zonDef;       /* -z  */
    real  lonDef;       /* -l  */
    ...
    real  attempDef;       /* -T  */
#ifdef KDE
    char *tzfile;       /* Default time zone file */
    _bool fTz4chart;    /* Do we also use default zoneinfo for charts ? */
    _bool fDst4Anim;    /* Do we use Daylight time for animations ? */
#endif
...
typedef struct _InternalSettings {
    ...
    _bool fDavison;   /* Have we already had a ephem file not found error? */
    ...
    real JD;            /* Julian day for current chart.          */
    real JDr;            /* Fractional Julian day for current chart.          */
...
#ifdef KDE
    char *bufComment;   /*buffer for input comment storing */
    _bool readComments;  /* yes if we must record the comments */
#endif
} IS;
...
typedef struct _ChartInfo {
    ...
    real zon;  /* Time zone        */
    real lon;  /* Longitude        */
    real lond;  /* Longitude to be trashed in matrix.c */
    real lat;  /* Latitude         */
    real latd;  /* Latitude to be trashed in matrix.c   */
    real alt;  /* Altitude in m   */
...
#endif
#ifdef KDE
    int portx, porty; /* x y margins offset for portrait printing. */
    int landx, landy; /* x y margins offset for landscape printing. */
#endif
} GS;
...
#ifdef KDE
    _bool painter;
    _bool fiskde;         /* is display active ? */
    _bool fixcour;        /* is "courier" the text font ? */
    int w;                /* width of character */
    int newHeight;        /* set whenever the drawing goes beyond the display */
    int nTimerDelay;      /* actual delay between frames redisplay */
    _bool longhms;         /* is the longitude expressed in hour, min, sec ?  */
#endif
